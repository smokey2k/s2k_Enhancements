...
local function ActivateControl(widget, event, ...)
	...
	if not validated or type(validated) == "string" then
		...
		if option.type == "color" then
			-- The color swatch already updates itself; rebuilding the whole panel
			-- here makes every control visibly jump when the picker closes.
		...
		elseif option.type == "toggle" then
			-- The checkbox already reflects its new value. Options which change
			-- another control's availability explicitly call NotifyChange.
		...
		if not hidden then
			if v.type == "group" then
				...
				elseif v.type == "toggle" then
					local controlType = v.dialogControl or v.control or "CheckBox"
					control = gui:Create(controlType)
					control:SetLabel(name)
				...
				elseif v.type == "range" then
					local controlType = v.dialogControl or v.control or "Slider"
					control = gui:Create(controlType)
					control:SetLabel(name)
				...
				elseif v.type == "color" then
					local controlType = v.dialogControl or v.control or "ColorPicker"
					control = gui:Create(controlType)
					control:SetLabel(name)
				...
				if control then
					...
					if S2KEnhancementsConfigControlBound then
						S2KEnhancementsConfigControlBound(control, appName, path, v)
					end
