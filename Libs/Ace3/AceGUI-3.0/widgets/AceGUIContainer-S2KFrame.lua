...
do
local Type,Version="S2KTextViewerWindow",4
...
local methods={OnAcquire=function(self)self.frame:Show()end,OnRelease=function(self)self.frame:Hide();self:SetText("");self:SetAction();self:SetNameField()end,SetTitle=function(self,t)self.title:SetText(t or"")end,SetText=function(self,t)self.editBox:SetText(t or"");self.editBox:SetCursorPosition(0);self.scroll:SetVerticalScroll(0);Update(self)end,SetNameField=function(self,label,value)self.scroll:ClearAllPoints();if label then self.nameLabel:SetText(label);self.nameBox:SetText(value or"");self.nameLabel:Show();self.nameBox:Show();self.scroll:SetPoint("TOPLEFT",18,-82)else self.nameLabel:Hide();self.nameBox:Hide();self.nameBox:SetText("");self.scroll:SetPoint("TOPLEFT",18,-42)end;self.scroll:SetPoint("BOTTOMRIGHT",-42,48);Update(self)end,SetAction=function(self,label,callback)self.actionCallback=callback;if callback then self.action:SetText(label or"OK");self.action:Show()else self.action:Hide()end end,Scroll=function(self,d)local s=self.scroll;s:SetVerticalScroll(math.max(0,math.min(s:GetVerticalScrollRange()or 0,(s:GetVerticalScroll()or 0)+d)));self.editBox:SetFocus()end,UpdateContent=Update}
local function Constructor()
 ...
 local nameLabel=f:CreateFontString(nil,"OVERLAY","GameFontNormalSmall");nameLabel:SetPoint("TOPLEFT",20,-43);nameLabel:Hide();local nameBox=CreateFrame("EditBox",nil,f,"InputBoxTemplate");nameBox:SetHeight(24);nameBox:SetPoint("TOPLEFT",20,-54);nameBox:SetPoint("RIGHT",-20,0);nameBox:SetAutoFocus(false);nameBox:Hide();local scroll=CreateFrame("ScrollFrame",nil,f,"UIPanelScrollFrameTemplate");scroll:SetPoint("TOPLEFT",18,-42);scroll:SetPoint("BOTTOMRIGHT",-42,48);scroll:EnableMouseWheel(true);local edit=CreateFrame("EditBox",nil,scroll);edit:SetMultiLine(true);edit:SetAutoFocus(false);edit:EnableMouse(true);edit:SetFontObject(ChatFontNormal);edit:SetTextInsets(4,4,4,4);scroll:SetScrollChild(edit)
 local measure=f:CreateFontString(nil,"ARTWORK");measure:SetFontObject(ChatFontNormal);measure:SetJustifyH("LEFT");measure:SetJustifyV("TOP");measure:Hide();local up=CreateFrame("Button",nil,f,"UIPanelButtonTemplate");up:SetSize(88,24);up:SetPoint("BOTTOMLEFT",18,16);local down=CreateFrame("Button",nil,f,"UIPanelButtonTemplate");down:SetSize(88,24);down:SetPoint("LEFT",up,"RIGHT",8,0);local action=CreateFrame("Button",nil,f,"UIPanelButtonTemplate");action:SetSize(150,24);action:SetPoint("BOTTOMRIGHT",-18,16);action:EnableMouse(true);action:SetHitRectInsets(0,0,0,0);action:SetFrameLevel(f:GetFrameLevel()+10);action:Hide()
 local resize=CreateFrame("Button",nil,f);resize:SetSize(20,20);resize:SetPoint("BOTTOMRIGHT",-8,8);resize:SetFrameLevel(f:GetFrameLevel()+5);resize:SetNormalTexture("Interface\\ChatFrame\\UI-ChatIM-SizeGrabber-Up");resize:SetHighlightTexture("Interface\\ChatFrame\\UI-ChatIM-SizeGrabber-Highlight");resize:SetPushedTexture("Interface\\ChatFrame\\UI-ChatIM-SizeGrabber-Down")
 local w={type=Type,frame=f,title=title,nameLabel=nameLabel,nameBox=nameBox,scroll=scroll,editBox=edit,measure=measure,up=up,down=down,action=action};for k,v in pairs(methods)do w[k]=v end;f.obj=w;edit.s2kViewer=w
 close:SetScript("OnClick",function()f:Hide()end);action:SetScript("OnClick",function()if w.actionCallback then w.actionCallback(w.editBox:GetText()or"")end end);nameBox:SetScript("OnEnterPressed",function(self)self:ClearFocus();if w.actionCallback then w.actionCallback(w.editBox:GetText()or"")end end);nameBox:SetScript("OnEscapePressed",function(self)self:ClearFocus()end);f:SetScript("OnDragStart",f.StartMoving);f:SetScript("OnDragStop",function(self)self:StopMovingOrSizing();w:Fire("OnGeometryChanged")end);f:SetScript("OnSizeChanged",function()Update(w)end);scroll:SetScript("OnMouseWheel",function(_,d)w:Scroll(d>0 and -48 or 48)end);up:SetScript("OnClick",function()w:Scroll(-96)end);down:SetScript("OnClick",function()w:Scroll(96)end);edit:SetScript("OnEscapePressed",function(self)self:ClearFocus()end);edit:SetScript("OnTextChanged",function()Update(w)end);edit:SetScript("OnMouseDown",function(self,b)if b=="LeftButton"then self.s2kSelecting=true;self.s2kScrollElapsed=0;self:SetScript("OnUpdate",SelectionUpdate)end end);edit:SetScript("OnMouseUp",StopSelection);edit:SetScript("OnHide",StopSelection);resize:SetScript("OnMouseDown",function(_,b)if b=="LeftButton"then f:StartSizing("BOTTOMRIGHT")end end);resize:SetScript("OnMouseUp",function()f:StopMovingOrSizing();w:Fire("OnGeometryChanged")end)
